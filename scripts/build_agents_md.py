...
def _capability_summaries(spec_tag: str) -> str:
    ...
    sections = [
        ...
        (
            ...
            + "not-yet), read the `conformance.toml` manifest. It ships inside this "
            + "package, beside this file, so it resolves whether you installed from "
            + "PyPI or are working in a clone: "
            + "`importlib.resources.files('openarmature') / 'conformance.toml'`. "
            + "Check it before planning against a behaviour from an accepted "
            + "proposal: `partial` entries say which half is missing, and this "
            + "release has two. "

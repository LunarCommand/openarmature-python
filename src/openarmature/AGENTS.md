...
- [Observability concept page](https://openarmature.ai/concepts/observability/): the
  `NodeEvent` shape, `started` / `completed` lifecycle.
- [Caller-supplied trace identifiers](#caller-supplied-trace-identifiers):
...
migration; see [Checkpointing](https://openarmature.ai/concepts/checkpointing/)),
...
- Migrations need to call the LLM or do other slow / fallible work.
  ...
  [bypass-if-output-exists](#bypass-if-output-exists), not in a
  migration callable.
...
- [Checkpointing concept page](https://openarmature.ai/concepts/checkpointing/):
  checkpointer backends and the resume contract.
- [`session-as-checkpoint-resume`](#session-as-checkpoint-resume):
...
`install_log_bridge` detects either handler class against the same provider and skips its own `addHandler` accordingly; the `openarmature.correlation_id` LogRecord factory still installs. On that skip path it **re-classes the handler the application already attached**, and warns that it did. Neither OTel handler populates the `EventName` field, so without the re-class OA's diagnostic event names would survive only as attributes in exactly the setup described above. Re-classing rather than replacing keeps the application's level, filters and formatter; what changes is the object's class, so `type()` on their handler reports a class they did not write. That is worth knowing before it surprises someone, which is why it warns rather than doing it quietly. Attaching OA's bridge before your own handler, or attaching yours to a different `LoggerProvider`, avoids it entirely: the retrofit is provider-scoped like the dedup check that reaches it, so a handler feeding a separate logs pipeline is never touched. The announcement goes out through `warnings` as well as the log, because the log record travels through the very handler being modified and a handler capped above WARNING would swallow it. It carries its own category, `LoggingSetupModified` (exported from `openarmature.observability.otel`), so a caller who accepts the change can silence just this warning with `warnings.simplefilter("ignore", LoggingSetupModified)` rather than every `UserWarning` in the process, and one who wants it fatal can pass `"error"` instead. On a handler whose private `_translate` seam is gone or has changed shape OA modifies nothing and warns instead, leaving the attributes-only behaviour rather than raising out of every log call in the process. The check is provider-scoped, so an application that intentionally attaches a handler against a DIFFERENT `LoggerProvider` (a separate logs pipeline) still gets the OA bridge against the OA provider; the helper only dedups when the SAME provider would receive duplicate emissions.

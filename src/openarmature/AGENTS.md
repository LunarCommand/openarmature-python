...
`install_log_bridge` detects either handler class against the same provider and skips its own `addHandler` accordingly; the `openarmature.correlation_id` LogRecord factory still installs. On that skip path it re-classes the handler the application already attached, so OA's diagnostic event names still reach the OTel `LogRecord`'s `EventName` field: neither OTel handler populates that field, and without the re-class a name would survive only as an attribute in exactly the setup described above. Re-classing rather than replacing keeps the application's level, filters and formatter. It declines silently on a handler whose private `_translate` seam is gone or has changed shape, which leaves the attributes-only behaviour rather than raising out of every log call in the process. The check is provider-scoped, so an application that intentionally attaches a handler against a DIFFERENT `LoggerProvider` (a separate logs pipeline) still gets the OA bridge against the OA provider; the helper only dedups when the SAME provider would receive duplicate emissions.
